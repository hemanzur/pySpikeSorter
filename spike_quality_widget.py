...
import sys
import numpy as np
from m_Spike_Utils import xCorr
#%%
class Sorting_Quality_Widget(QtGui.QWidget):
    ...
    def plotXCorr(self):
        ...
        for j, k in enumerate(units2Plot):
            ax = self.mainFig.figure.add_subplot(nRows, 8, j+1)
            
            chan = k[0:8]
            unit = k[9:]
            ax.set_title(chan+' '+unit, color = 'w')
            #get the timestamps for that unit
            ...
            r, t = xCorr(baseUnitTS, UnitTS, binSize = 1, Win = [0, 600000])
