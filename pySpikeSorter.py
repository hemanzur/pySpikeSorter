...
class SpikeSorter(QtGui.QMainWindow):
    ...
    def PlotOverview(self):
        ...
        c = list(range(self.OverviewTab2['OverviewTable'].rowCount()))
        c.reverse()
        for k in c:
        ...
        for j, k in enumerate(chanNodes):
            ...
            if k.__contains__('isTrash'):
                if k.isTrash.read():
                    ...
                    self.OverviewTab1['Figure'].figure.axes[j].set_facecolor([.5, .5, .5])
        ...
        for k in node:
            ...
            if 'Unit' in k._v_name and 'Indx' in k:
                if k.Indx.nrows >= nEvents:
                    indx = k.Indx.read(start=0, stop=k.Indx.nrows, step=int(k.Indx.nrows / nEvents))
                else:
                    indx = k.Indx.read()
            elif 'Unsorted' in k._v_name:
                if k.nrows >= nEvents:
                    indx = k.read(start=0, stop=k.nrows, step=int(k.nrows / nEvents))
                else:
                    indx = k.read()
    ...
    def PlotChanProc(self):
        ...
        self.WaveformXax = list(range(self.WfSize))
    ...
    def setTrash_proc(self):
        ...
        if sender.checkState() in [1, 2]:
            ...
            self.OverviewTab1['Figure'].figure.axes[indx].set_facecolor('y')
        elif sender.checkState() == 0:
            self.h5file.create_array(nodeName, 'isTrash', False)
            self.OverviewTab1['Figure'].figure.axes[indx].set_facecolor('w')
    ...
    def TrashChans_proc(self):
        ...
        for l, k in enumerate(chans):
            ...
            if k.TimeStamp.nrows < n:
                self.OverviewTab1['Figure'].figure.axes[l].set_facecolor('y')
                ...
                self.OverviewTab1['Figure'].figure.axes[l].set_facecolor('w')
    ...
    def NearestPoint(self, event):
        ...
        if event.button == 3 and event.inaxes and self.ChanTab['FeaturesFigNtb'].mode == '':
            ...
            self.SampleWaveform.set_data(list(range(self.WfSize)),
                                         self.CurWaveforms[self.dataIndx[res], :])
    ...
    def PlotFeatures(self):
        ...
        if What2Plot in ['All Waveforms', 'Sorted']:
            self.dataIndx = list(range(self.CurTs.size))
            pc = self.ChanTab['PCA']
        ...
        # to plot the unsorted channels
            ...
            if lx > self.nPtsSpin.value():
                indx = list(range(0, lx, lx / self.nPtsSpin.value()))
            else:
                indx = list(range(lx))
        ...
        self.XYData = cKDTree(np.array([x, y]).T)
...
#        self.ChanTab['WavesFigure'].figure.canvas.draw()
    ...
    def LassoCallback_AddUnit(self, verts):
        ...
        if lunsort > self.nPtsSpin.value():
            indx = list(range(0, lunsort, lunsort / self.nPtsSpin.value()))
        else:
            indx = list(range(lunsort))
        ...
        if lunit > self.nPtsSpin.value():
            indx = list(range(0, lunit, lunit / self.nPtsSpin.value()))
        else:
            indx = list(range(lunit))

        ax.plot(unitData[:, 0][indx], unitData[:, 1][indx],
                linestyle = '',
                marker = ',',
                mfc = list(self.UnitColors[self.NUnits]),
                mec = list(self.UnitColors[self.NUnits]),
    ...
    def LassoCallback_AddRegion(self, verts):
        ...
        self.verts = np.array(verts)
        if n > 25:
            self.verts = self.verts[list(range(0, n, n / 25))]
    ...
    def UnitsTable_AddUnit(self, unitName):
        ...
        for k in list(range(self.ChanTab['UnitTabsWidget'].count())):
    ...
    def ResetChannelTab_proc(self):
        ...
        tabs = list(range(self.ChanTab['UnitTabsWidget'].count()))
        tabs.reverse()
        if len(tabs) > 0:
            for k in tabs:
